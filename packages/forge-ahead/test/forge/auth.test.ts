...
describe("getAuthForRequest", () => {
  ...
  it("returns asUser(raiseOnBehalfOf) when raiseOnBehalfOf is a string", () => {
    const body = { raiseOnBehalfOf: "user-789" };

    getAuthForRequest(body);

    expect(asUser).toHaveBeenCalledWith("user-789");
    expect(asApp).not.toHaveBeenCalled();
  });

  it("returns asApp() when raiseOnBehalfOf is absent", () => {
    const body = { issueType: "Bug" };
  ...
  it("returns asApp() when body is null", () => {
    getAuthForRequest(null);

    expect(asApp).toHaveBeenCalledTimes(1);
    expect(asUser).not.toHaveBeenCalled();
  });

  it("returns asApp() when body is a primitive string", () => {
    getAuthForRequest("not-an-object");

    expect(asApp).toHaveBeenCalledTimes(1);
    expect(asUser).not.toHaveBeenCalled();
  });

  it("returns asApp() when raiseOnBehalfOf is not a string", () => {
    const body = { raiseOnBehalfOf: 12345 };

...
/**
 * Select the auth client for a raise-on-behalf-of request.
 *
 * When the request body contains a `raiseOnBehalfOf` string field, this
 * function returns `asUser(raiseOnBehalfOf)` so that the Jira issue is
 * raised as that user.  Any other input (missing field, non-string, invalid
 * body shape) falls back to `asApp()`.
 *
 * @param body - The raw request body (parsed JSON or unknown)
 * @returns An authenticated Forge API client
 */
export function getAuthForRequest(body: unknown): ForgeAuthClient {
  if (
    body !== null &&
    typeof body === "object" &&
    "raiseOnBehalfOf" in body &&
    typeof (body as Record<string, unknown>).raiseOnBehalfOf === "string"
  ) {
    const accountId = (body as Record<string, unknown>)
      .raiseOnBehalfOf as string;
    return asUser(accountId);
  }
  return asApp();
}
